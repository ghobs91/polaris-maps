...
1. **Navigation sync** — mirrors the phone's active navigation view to CarPlay in real time:
   ...
   - Chrome: guidance banner tint matching the phone's `NextTurnBanner`, an auto-hidden navigation bar (Apple Maps-style full-bleed map), a mute navigation-bar button wired to the phone's `muted` state, and a route-overview button (`mapViewHost.showRouteOverview()`)
