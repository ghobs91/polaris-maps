...
| `websiteActionsService.ts` | Fetches the venue homepage and detects **order / reserve / menu** links (JSON-LD `hasMenu`/`potentialAction`, known platforms like DoorDash/OpenTable/Resy, then URL/text keywords) for one-tap action pills on the place card. In-memory cached, http(s)-only.                                                                                                                                                                              |
