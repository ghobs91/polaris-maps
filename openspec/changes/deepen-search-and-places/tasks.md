...
- [ ] 1.1 Define `SearchFilters` and `SortOption` types and a session-scoped `searchViewStore` (Zustand) holding `filters` and `sort`
- [ ] 1.2 Implement `src/services/search/resultFilter.ts`: pure `applyFilters(results, filters)` covering open now, min rating, max price, max distance, categories
- [ ] 1.3 Implement `src/services/search/resultSort.ts`: pure comparators for relevance, distance, rating, price, with distance tie-break
- [ ] 1.4 Seed default filters from parsed intent (`wantsOpenNow` / `wantsQuality` / `wantsCheap`) and let explicit user selections override them
...
- [ ] 1.10 Unit tests: each predicate, unknown-data pass-through, comparator ordering and tie-breaks, intent-seeded defaults, explicit override
- [ ] 1.11 Unit tests: filters re-applied on a later stage emission; results never reappear unfiltered; scores unchanged by filtering
- [ ] 1.12 Integration test: apply filters plus sort in the search tab and confirm the rendered list order and membership
