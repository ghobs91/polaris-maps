...
const US_CITY_COORDS = new Map([
  ...
  ['neah bay, wa', [48.3681, -124.625]],
  ['nespelem, wa', [48.1696, -118.975]],
  ...
  ['newport news, va', [36.9785, -76.428]],
  ...
  ['saint helens, or', [45.864, -122.8065]],
...
for (let i = 1; i < lines.length; i++) {
  ...
  const uzaPop =
    parseInt(row['Primary UZA Population']?.replace(/^"|"$/g, '').replace(/,/g, ''), 10) || 0;
...
console.log(
  `After dedup: ${deduped.size} unique feeds (${feedEntries.length - deduped.size} duplicates removed)`,
);
